...
export class Prismtower extends Structure {
    ...
    clearXray() {
        ...
        let raylength = this.pointDistance(this.positions,this.attacktarget.currentclickpoints,true),
            verticee2s={
                x:this.attacktarget.currentclickpoints.x - this.positions.x,
                y:this.attacktarget.currentclickpoints.y - this.positions.y
            },
            sintheta = verticee2s.x/(this.squalCaculator(verticee2s,true))?verticee2s.x/(this.squalCaculator(verticee2s,true)):0,
            costheta =  verticee2s.y/(this.squalCaculator(verticee2s,true))?verticee2s.x/(this.squalCaculator(verticee2s,true)):0,
            sixquoterx = sintheta*raylength/6,
            sinthetay = costheta*raylength/6;
        let timerss = setInterval(() => {
            if (index < 6) {
                this.ctx.moveTo(this.positions.x+this.size.x*0.5, this.positions.y+10);       //设置起点状态
                this.ctx.lineTo(this.positions.x+index*sixquoterx, this.positions.y+index*sinthetay);       //设置末端状态
    ...
    clearLine(sintheta,costheta) {
       ...
       let  raylength = this.pointDistance(this.positions,this.attacktarget.currentclickpoints,true),
            retanclewidth = (2 / sintheta)?(2 / sintheta):0,
            retancleheight = (2 * 2 / costheta)?(2 * 2 / costheta):0,
            counst = 600 / retanclewidth,

            cleartimes = 0;
        for (let j = 0; j < counst; j++) {
            let x = retanclewidth * cleartimes + 100,
                y = (((sintheta/costheta)?sintheta/costheta:0)*x)+ - (0.25) * retancleheight,
                width = retanclewidth,
